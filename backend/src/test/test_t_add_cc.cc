...
RamDictionary<Tensor<std::optional<PtVal>>> CreateAddCCCheck(
...
Dag<TOp> CreateAddCCTOpDag() {
  ...
  const auto& add_cc =
      top_dag.AddNode(std::make_unique<TAddCC>(input_layout), {a, b});
  MakeOutputNode(top_dag, add_cc, "out0");
  return top_dag;
}

TEST(TAddCCTest, Basic) {
  DoTest<Cleartext>(CreateAddCCTOpDag, CreateAddCCCheck);
}
