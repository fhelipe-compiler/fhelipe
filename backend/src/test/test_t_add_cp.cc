...
RamDictionary<Tensor<std::optional<PtVal>>> CreateAddCPCheck(
    const Dictionary<Tensor<PtVal>>& tensor_dict) {
  RamDictionary<Tensor<std::optional<PtVal>>> result;
  auto inputs = GetValues(tensor_dict);
  result.Record("out0", ToOptionalTensor({inputs[0].GetShape(),
                                          Estd::transform(inputs[0].Values(),
                                                          inputs[1].Values(),
                                                          std::plus<>())}));
  return result;
}

Dag<TOp> CreateAddCPTOpDag() {
  auto input_layout = RandomLayout();
  Dag<TOp> top_dag;
  const auto& a = MakeInputNode(top_dag, input_layout, "in0");
  const auto& add_cp = top_dag.AddNode(
      std::make_unique<TAddCP>(input_layout, "tensor", LogScale(50)), {a});
  MakeOutputNode(top_dag, add_cp, "out0");
  return top_dag;
}

TEST(TAddCPTest, Basic) {
  DoTest<Cleartext>(CreateAddCPTOpDag, CreateAddCPCheck);
}
