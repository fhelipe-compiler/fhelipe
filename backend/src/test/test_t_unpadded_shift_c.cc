...
Tensor<std::optional<PtVal>> ComputeShifted(const Tensor<PtVal>& tensor,
  ...
  Tensor result(tensor.GetShape(),
                std::vector<std::optional<PtVal>>(shape.ValueCnt()));
  for (int flat_idx : Estd::indices(shape.ValueCnt())) {
    auto non_cyclic_add = shift_by.NonCyclicAdd(TensorIndex(shape, flat_idx));
    if (non_cyclic_add.has_value()) {
      result[non_cyclic_add.value()] = tensor[TensorIndex(shape, flat_idx)];
    }
  }
  return result;
}

RamDictionary<Tensor<std::optional<PtVal>>> CreateUnpaddedShiftCCheck(
...
Dag<TOp> CreateUnpaddedShiftCTOpDag() {
  ...
  const auto& node = top_dag.AddNode(
      std::make_unique<TUnpaddedShiftC>(input_layout, shift_by), {a});
  MakeOutputNode(top_dag, node, "out0");
  return top_dag;
}

TEST(TUnpaddedShiftCTest, Basic) {
  DoTest<Cleartext>(CreateUnpaddedShiftCTOpDag, CreateUnpaddedShiftCCheck);
}
