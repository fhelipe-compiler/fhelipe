...
bool RequiresLazyBootstrapping(const LeveledTOp& new_node) {
  return new_node.GetLevelInfo().Level() == kMinLevel;
}

std::shared_ptr<Node<LeveledTOp>> BootstrapIfNeeded(
    Dag<LeveledTOp>& dag, const std::shared_ptr<Node<LeveledTOp>>& node,
    const LevelInfo& bootstrapped_level_info) {
  if (RequiresLazyBootstrapping(node->Value())) {
...
std::shared_ptr<Node<LeveledTOp>> BuildNewNode(
  ...
  return BootstrapIfNeeded(dag, new_node,
                           LevelInfo(usable_levels, ct_log_scale));
}

}  // namespace

LevelingPassOutput LazyBootstrappingPass::DoPass(
